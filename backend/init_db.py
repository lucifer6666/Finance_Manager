...
import os
import sys
from sqlalchemy import inspect, Column, String, Float, Date, Integer
...
if db_exists:
    ...
    if 'savings_investments' in existing_tables:
        ...
        if columns_to_add:
            print(f"  • Found {len(columns_to_add)} missing columns")
                ...
                with engine.begin() as connection:
                    for col_name in columns_to_add:
                        if col_name == 'is_recurring':
                            connection.execute(f'ALTER TABLE savings_investments ADD COLUMN {col_name} INTEGER DEFAULT 0')
                            print(f"    ✓ Added column: {col_name}")
                        elif col_name == 'recurring_type':
                            connection.execute(f'ALTER TABLE savings_investments ADD COLUMN {col_name} VARCHAR')
                            print(f"    ✓ Added column: {col_name}")
                        elif col_name == 'recurring_amount':
                            connection.execute(f'ALTER TABLE savings_investments ADD COLUMN {col_name} FLOAT')
                            print(f"    ✓ Added column: {col_name}")
                        elif col_name == 'last_recurring_date':
                            connection.execute(f'ALTER TABLE savings_investments ADD COLUMN {col_name} DATE')
                            print(f"    ✓ Added column: {col_name}")
